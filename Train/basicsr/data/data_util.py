...
def paired_paths_from_folder(folders, keys, filename_tmpl):
    ...
    Args:
        folders (list[str]): List of folder paths. The order should correspond to keys.
        keys (list[str]): List of keys identifying folders (e.g., ['lq', 'mid', 'gt']).
        filename_tmpl (str): Template for input folder filenames (excluding extension).

    Returns:
        list[dict]: List of dictionaries containing paths for each key.
    """
    assert len(folders) == len(keys), (
        f'Number of folders ({len(folders)}) and keys ({len(keys)}) must match.')
    assert len(folders) >= 2, 'At least two folders are required.'

    # Scan all folders to collect file lists
    paths_list = []
    ...
    num_files = len(paths_list[0])
    for i, paths in enumerate(paths_list[1:]):
        assert len(paths) == num_files, (
            f'{keys[i+1]} has {len(paths)} files vs {keys[0]} has {num_files}.')

    paths = []
    for idx in range(num_files):
        pair = {}
        # Extract basename from the last folder (typically 'gt')
        last_folder_path = paths_list[-1][idx]
        basename_last = osp.splitext(osp.basename(last_folder_path))[0]

        # Process each folder
        for folder_idx, (folder, key) in enumerate(zip(folders, keys)):
            if folder_idx == 0:
                # Generate input filename using template and last folder's basename
                input_filename = paths_list[0][idx]
                _, ext = osp.splitext(input_filename)
                generated_name = f"{filename_tmpl.format(basename_last)}{ext}"
                assert generated_name in paths_list[0], (
                    f'{generated_name} not found in {keys[0]} paths.')
                full_path = osp.join(folder, generated_name)
                pair[f'{key}_path'] = full_path
            else:
                # Directly use filename for other folders
                filename = paths_list[folder_idx][idx]
                full_path = osp.join(folder, filename)
                pair[f'{key}_path'] = full_path
        paths.append(pair)
    return paths

def paired_paths_from_folder_param(folders, keys):
    """基于文件名完全匹配的多文件夹路径配对
    
    Args:
        folders (list[str]): 文件夹路径列表，顺序与keys对应
        keys (list[str]): 文件夹标识（如['lq', 'gt', 'sr', 'param']）
    Returns:
        list[dict]: 配对后的路径字典列表
    """
    assert len(folders) == len(keys), "文件夹数量与keys数量必须一致"
    assert len(folders) >= 2, "至少需要2个文件夹"
    
    # 1. 获取所有文件夹的文件名（不带路径和扩展名）
    file_bases = []  # 每个元素为：{文件夹索引: {文件名基底: 完整文件名}}
    for folder in folders:
        # 获取文件夹内所有文件的"基底名"（不含扩展名）和完整文件名
        base_to_file = {}
        for filename in scandir(folder):
            base = osp.splitext(filename)[0]  # 去除扩展名，保留基底
            base_to_file[base] = filename
        file_bases.append(base_to_file)
    
    # 2. 以第一个文件夹（通常建议是'gt'，可根据实际调整）为基准
    # 这里假设keys中'gt'的索引为1，若你的基准是其他key，需修改此处
    ref_idx = keys.index('gt')  # 以'gt'文件夹为基准
    ref_bases = set(file_bases[ref_idx].keys())  # 基准文件夹的所有文件名基底
    
    # 3. 检查其他文件夹是否包含所有基准基底
    for i, key in enumerate(keys):
        if i == ref_idx:
            continue
        current_bases = set(file_bases[i].keys())
        missing = ref_bases - current_bases
        assert not missing, f"{key}文件夹缺少文件：{missing}（基准：{keys[ref_idx]}）"
    ...
    for base in ref_bases:
        ...
        for i, key in enumerate(keys):
            # 每个文件夹中取与基准基底同名的文件
            filename = file_bases[i][base]
            pair[f"{key}_path"] = osp.join(folders[i], filename)
